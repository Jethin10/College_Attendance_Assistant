...
- Scrapes both the legacy `attended / held` table and the current column-based
  NIET layout (`Present Count`, `Total Count`, and `Percentage`)
- Detects the selected semester and student name automatically, so changing
  terms replaces the old subject list with the current one
...
- Subjects with no conducted classes are shown as `Not started` and do not
  distort the active-subject average
